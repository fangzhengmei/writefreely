...
4. [集合入口：四层路由与命中机制](#集合入口四层路由与命中机制)
5. [公开路径详解](#公开路径详解)
6. [用户集合管理](#用户集合管理)
7. [/me/posts/ 与 /me/c/ 的数据关联](#meposts-与-mec-的数据关联)
8. [文章列表](#文章列表)
9. [别名冲突处理：防御层 + 三层 Fallback](#别名冲突处理防御层--三层-fallback)
10. [总结](#总结)
...
## 别名冲突处理：防御层 + 三层 Fallback

WriteFreely 的别名系统面临多个命名空间的共享问题：
- 集合别名 vs 帖子 ID（都是 URL 路径段，路由层无法区分）
- 集合别名 vs 保留字/静态页面文件名
- 旧别名 vs 新别名（用户改名后，旧 URL 需要继续可用）

这些冲突通过 **一道防御层 + 三层 Fallback** 机制来处理。防御层在创建时阻断，Fallback 层在访问时补救。
...
前缀字符（@/~/$ 等）在别名系统中有一个特殊性质：**前缀只影响路由匹配，不参与别名的唯一性判断**。

- 集合别名存储在数据库中，**不包含前缀字符**
- 前缀只是 URL 风格，同一个集合可以通过 `/myblog/` 和 `/@myblog` 访问
- `collectionredirects` 表中只存别名，不存前缀信息

因此：
- 前缀入口（层次①）的 Fallback 逻辑与规范入口（层次②）完全相同，都走 `processCollectionPermissions`
- 旧别名重定向后，前缀会丢失（总是重定向到无前缀的规范 URL `/{newAlias}/`）
- 如果用户习惯了带前缀的 URL，改名后需要重新适应无前缀的新地址

---

### 防御层 vs Fallback 层：设计原因

| 层次 | 处理时机 | 目标场景 | 无法替代对方的原因 |
|------|----------|----------|-------------------|
| 防御层：创建时 | CreateCollection 阶段 | 阻止明显不合法的别名被创建 | 无法处理"先有帖子后有限制"的历史遗留；无法处理用户改名的旧链接；无法处理裸别名规范化 |
| Fallback 层：访问时 | 每次访问 404 时 | 裸别名规范化、历史帖子链接、旧别名重定向 | 性能开销大（每次 404 都要多查 1-2 张表）；无法给出友好的创建时错误提示 |

防御层减少运行时的 Fallback 次数（性能），Fallback 层保证 URL 长期稳定（SEO 和用户体验）。两者缺一不可。
...
1. **四层路由架构**：集合相关 URL 分布在四个层次上——前缀入口（层次①）、规范入口（层次②）、集合子资源（层次③）、根级 catch-all（层次④）。每层职责明确，按注册顺序依次匹配。

2. **`/myblog`（裸别名）的真实链路**：不走集合域的路由，而是匹配根级 `/{post}` → `handleViewPost` → 内部通过 `GetCollection` 检查识别为集合 → 301 重定向到规范 URL `/myblog/`。这是一种**数据库驱动的 URL 路由**设计。
...
5. **`/me/posts/` 与 `/me/c/` 的关联**：`viewArticles` 同时加载匿名文章和可发布集合，模板中通过 "move to" 操作调用 API 更新 `collection_id`，实现草稿→已发布的流转。

6. **别名冲突的防御 + 三层 Fallback**：
   - 防御层：保留字 → 帖子 ID → DB 唯一约束
   - Fallback 层：根级 catch-all（裸别名规范化）→ 集合视图（帖子 ID + 旧别名）→ 文章视图（旧别名+保留 slug）

7. **为什么 Fallback 要分三层**：每层只能构造自己知道的路径——根级不知道是集合还是帖子；集合视图只知道首页；文章视图知道 slug 所以能保留。三层各司其职，确保所有形态的旧 URL 都能正确跳转。

8. **前缀入口与别名系统的关系**：前缀只影响路由匹配（层次①），不参与别名唯一性判断和重定向存储；旧别名重定向后前缀会丢失，总是跳转到无前缀的规范 URL。
